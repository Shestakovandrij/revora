...
const schema = z.object({
  ...
  cargoLengthCm: z.coerce.number().min(0).optional(),
  cargoWidthCm: z.coerce.number().min(0).optional(),
  cargoHeightCm: z.coerce.number().min(0).optional(),
  cargoVolumeM3: z.coerce.number().min(0).optional(),
  cargoPlaces: z.coerce.number().min(0).optional(),
  loadingMethod: z.string().optional(),
  unloadingMethod: z.string().optional(),
  fragile: z.coerce.boolean().default(false),
  loadingHelp: z.coerce.boolean().default(false),
...
export async function createBookingAction(formData: FormData): Promise<BookingResult> {
  ...
  const order = await prisma.order.create({
    data: {
      ...
      cargoLengthCm: d.cargoLengthCm ?? null,
      cargoWidthCm: d.cargoWidthCm ?? null,
      cargoHeightCm: d.cargoHeightCm ?? null,
      cargoVolumeM3: d.cargoVolumeM3 ?? null,
      cargoPlaces: d.cargoPlaces ?? null,
      loadingMethod: d.loadingMethod || null,
      unloadingMethod: d.unloadingMethod || null,
      fragile: d.fragile,
      loadingHelp: d.loadingHelp,
